...
{
    return (int)index % self.imageURLStringsGroup.count;
}
...
//	cell.imageView.tag = indexPath.item;
    ...
    if ([self.delegate respondsToSelector:@selector(setupCustomCell:forIndex:cycleScrollView:)] &&
		...
		cell.playView.tag = indexPath.item;
		cell.playView.alpha = 0;
	}
    ...
    if (!cell.hasConfigured) {
		...
		cell.imageView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.7];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
	...
	if ([self.delegate respondsToSelector:@selector(cycleScrollViewDidScroll:withMainView:)]) {
		[self.delegate cycleScrollViewDidScroll:self withMainView:(UICollectionView *)scrollView];
	}
