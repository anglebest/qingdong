...
- (void)show:(BOOL)animation to:(UIViewController *)viewController withRects:(NSArray<NSValue *> *_Nullable)rects
{
	...
	self.source.minScale = self.minScale;
...
//		[MBProgressHUD y_showMsg:@"获取图片失败"];
    ...
    switch ([[TZImageManager manager] authorizationStatusAuthorized]) {
		case 0:
		{
			...
			[[TZImageManager manager] requestAuthorizationWithCompletion:^{
				...
				[[TZImageManager manager] savePhotoWithImage:image completion:^(PHAsset *asset, NSError *error) {
					...
					if (error) {
						[MBProgressHUD showMessage:@"保存失败" toView:cell];
//                        NSError *error = [NSError errorWithDomain:kDomain code:-400 userInfo:nil];
//                        [self.saveComplete sendNext:@{@"error":error}];
					} else {
                        [MBProgressHUD showMessage:@"保存成功" toView:cell];
