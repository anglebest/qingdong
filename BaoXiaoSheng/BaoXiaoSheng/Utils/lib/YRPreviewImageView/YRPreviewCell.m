...
- (void)bindLoadImage {
	...
	self.loadImageDisposable = [[RACSignal combineLatest:@[RACObserve(self, thumbnail), RACObserve(self, imagePath)] reduce:^id _Nullable(UIImage *thumb, NSString *imgPath){
		if (([imgPath isKindOfClass:[UIImage class]] || imgPath.length > 0) && thumb != nil) {
		...
		if (x != nil) {
			...
			if ([imgPath isKindOfClass:[NSString class]] && [imgPath.lowercaseString containsString:@"http"]) {
				// 原图是网络请求的，缩略图是本地的
				size = thumb.size;
				[[SDWebImageManager sharedManager] loadImageWithURL:[NSURL URLWithString:imgPath] options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
				...
				// 原图和缩略图都是本地的
				if ([imgPath isKindOfClass:[UIImage class]]) {
					size = ((UIImage *)imgPath).size;
					self.imageView.image = (UIImage *)imgPath;
				} else {
