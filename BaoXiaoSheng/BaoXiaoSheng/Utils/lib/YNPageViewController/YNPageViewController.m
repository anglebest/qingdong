...
- (void)setupPageScrollView {
    ...
    CGFloat contentHeight = kYNPAGE_SCREEN_HEIGHT - navHeight - tabHeight - cutOutHeight + 10;
    ...
    if ([self isSuspensionTopPauseStyle]) {
        ...
        //self.controllersM.count自己改的 self.controllersM.count-1 设置滚动范围，最后一页不滚动
        self.pageScrollView.contentSize = CGSizeMake(kYNPAGE_SCREEN_WIDTH * (self.controllersM.count-1), self.pageScrollView.yn_height);
        ...
        //self.controllersM.count自己改的 self.controllersM.count-1 设置滚动范围，最后一页不滚动
        self.pageScrollView.contentSize = CGSizeMake(kYNPAGE_SCREEN_WIDTH * (self.controllersM.count-1), contentHeight - ([self isTopStyle] ? self.config.menuHeight : 0));
